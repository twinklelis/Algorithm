...
public class largeFigure {
	...
	public static void main(String[] args) {
		...
		int grade = 1;
		for(int i = 0; i < num; i++) {
			String[] bodyArr = scanner.nextLine().split(" ");
			weightArr[i] = Integer.parseInt(bodyArr[0]);
			heightArr[i] = Integer.parseInt(bodyArr[1]);
		}
		
		for(int i = 0; i < num; i++) {
			for(int k = 0; k < num; k++) {
				if(weightArr[k] > weightArr[i] && heightArr[k] > heightArr[i]) {
					grade+=1;
				}
			}
